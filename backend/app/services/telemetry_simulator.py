...
class TelemetryActor:
    ...
    def step(self, dt: float) -> None:
        ...
        # Soft-bounce inside the demo area so tracks stay in view
        if abs(self.lat - CENTER_LAT) > _meters_to_deg_lat(2200):
            self.heading = (self.heading + 180) % 360
            self.route_target = None
        if abs(self.lon - CENTER_LON) > _meters_to_deg_lon(2200, self.lat):
            self.heading = (self.heading + 180) % 360
            self.route_target = None
        self.last_seen = now
...
class TelemetrySimulator:
    ...
    def randomize_airspace(self) -> List[str]:
        """Reset to a mixed airspace of 8-15 persistent actors."""
        rng = random.Random()
        self.actors.clear()
        n = rng.randint(8, 15)
        ids = []
        # Keep roughly half authorized, half unregistered; mixed types
        for i in range(n):
            roll = rng.random()
            if roll < 0.55:
                auth, rtype = "AUTHORIZED", rng.choice(["drone", "drone", "aircraft", "bird"])
            else:
                auth, rtype = "UNREGISTERED", rng.choice(["drone", "drone", "bird", "aircraft"])
            a = TelemetryActor(actor_type=rtype, authorization=auth)
            self.actors[a.track_id] = a
            ids.append(a.track_id)
        # Make a couple of unregistered drones headed into the red coastal zone
        unreg_drones = [a for a in self.actors.values()
                        if a.authorization == "UNREGISTERED" and a.actor_type == "drone"]
        for a in unreg_drones[:2]:
            a.route_target = {"lat": CENTER_LAT + _meters_to_deg_lat(rng.uniform(-200, 200)),
                              "lon": CENTER_LON + _meters_to_deg_lon(rng.uniform(-200, 200), CENTER_LAT),
                              "altitude_m": rng.uniform(40, 120)}
        self._last_step = time.time()
        return ids

    def spawn(self, actor_type: str = "drone", authorization: str = "UNREGISTERED",
              route_into_center: bool = False) -> TelemetryActor:
        if len(self.actors) >= 15:
